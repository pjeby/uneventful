...
describe("Sources", () => {
    ...
    describe("emitter()", () => {
        it("should emit/close the source when called", () => {
            ...
            connect(e.source, log.emit).onEnd(logClose); runPulls();
    ...
    describe("empty()", () => {
        it("immediately closes", () => {
            ...
            const s = empty()
            const c = new Conduit().onEnd(logClose);
    ...
    describe("fromAsyncIterable()", () => {
        it("should output all the values, then close", async () => {
            ...
            // When it's subscribed
            const c = connect(s, log.emit).onEnd(logClose);
        ...
        it("should pause and resume per protocol", async () => {
            ...
            const c = connect(s, v => (log(v), v !== 3)).onEnd(logClose);
            see(); runPulls();
