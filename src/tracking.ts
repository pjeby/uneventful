...
export interface Job<T=any> extends Yielding<T>, Promise<T> {
    ...
    /**
     * Like {@link Job.must}, except a function is returned that will *remove*
     * the cleanup function from the job, if it's still present. (Also, the
     ...
     * {@link getJob} will return it, etc.  (Note: signal dependency tracking
     ...
     * to another job, event handler, etc.
     */
    readonly end: () => void;
